...
abstains on **93%** of them. The cause is upstream of the geometry: the caption parser extracts no
usable letter run from **50.7%** of real captions, versus 7.3% of synthetic ones, because real
captions write `at 2 months of age (A), 5 months (B)`, `in A), Y mice B)`, `young (A and E)`, or
bare `a, ai, aii`. It made zero silent errors across 259 invocations -- every answer it stood
behind matched the caption's own letter count -- but it stands behind almost nothing. The 95.1%
above is a statement about synthetic captions generated by the same script that drew the figures.
